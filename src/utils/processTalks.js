...
async function getTalks() {
  ...
  let talks = entries.map(({ id, slug, data }) => {
    ...
    const times = {
      ...
      newYork: datePrague.setZone('America/New_York').
          toFormat('MMM dd, HH:mm ccc'),
      shanghai: datePrague.setZone('Asia/Shanghai').
          toFormat('MMM dd, HH:mm ccc'),
    };
  ...
  // Sort by Prague date (earliest first), then reverse so most recent talks appear first
